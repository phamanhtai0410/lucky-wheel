...
const LuckyWheelGame = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user && user.id !== lastUserId) {
      ...
      console.log("User data changed, updating wallets in context:", user);
      console.log("Embedded Wallets:", user.embedded_wallets);
      console.log("Smart Accounts:", user.smart_accounts);
      
      // Populate wallet context with user's wallet data
      if (user.embedded_wallets && user.smart_accounts) {
        setUserWallets({
          embedded_wallets: user.embedded_wallets,
          smart_accounts: user.smart_accounts
        });
      }
    }
  }, [isAuthenticated, user, setUserWallets]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const spinWheel = async (chainId: number) => {
    ...
    try {
      ...
      // Call API to spin the wheel using selected wallet type
      const apiEndpoint = getAPIEndpoint();
      const isSmartAccount = selectedWallet === WalletType.SMART_ACCOUNT;
      
      // Prepare request body based on wallet type
      const requestBody = isSmartAccount ? {
        chainId: chainId,
        sponsor: true,
        transactionReq: {
          to: txRequest.to,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to initiate spin transaction"
        );
      }
      ...
      setTransactionState({
        status: `Transaction bundled with ${selectedWallet === WalletType.SMART_ACCOUNT ? 'Smart Account' : 'EOA Wallet'}, executing...`,
      ...
      const receipt = await waitForUserOperationReceipt(
        txData.data.userOpHash,
        chainId
      );
      console.log("receipt::", receipt.receipt);

      const spinResult = decodeSpinCompletedEvent(
        receipt.receipt,
        wheel_contract
      );
      ...
      const segmentOffset =
        segmentSize * (segments.length - 1 - winningSegmentIndex);
        ...
        <div className="p-6">
          <div className="mb-6 text-center">
            ...
            <p className="text-gray-600 mt-2">
              Spin the wheel to win G token prizes! All transactions secured
              on-chain using Account Abstraction.
            </p>
          </div>
              ...
              style={{
                background: `conic-gradient(
                  ${segments
                    .map(
                      (segment, i) =>
                        `${segment.color} ${i * (100 / segments.length)}% ${
                          (i + 1) * (100 / segments.length)
                        }%`
                    )
                    .join(", ")}
                )`,
          ...
          {result ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-6 text-gray-700 text-sm text-center">
                <p>All transactions processed via {selectedWallet === WalletType.SMART_ACCOUNT ? 'Smart Account' : 'EOA Wallet'}</p>
                <p className="text-emerald-600 font-medium">
                  Gas fees sponsored for all your spins!
                </p>
                <p className="text-xs text-gray-500 mt-2">
                  Using {getAPIEndpoint()} APIs
                </p>
              </div>
            </div>
          )}
