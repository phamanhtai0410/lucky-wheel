...
const Login = ({onLoginStart}: LoginProps) => {
  ...
  const handleOAuthLogin = async (provider: string) => {
    ...
    console.log("BUNDLER_ENDPOINT:::", getBundlerEndpoint(), onLoginStart);
    if (onLoginStart) {
      onLoginStart();
    }
    ...
    try {
      ...
      await axios.get(`${getBundlerEndpoint()}auth/init-client`, {
        headers: {
          "x-api-key": getApiKey(),
        },
      });

      const response = await axios.get(`${getBundlerEndpoint()}auth/${provider}`, {
        method: "GET",
        headers: {
          "x-api-key": getApiKey(),
        },
